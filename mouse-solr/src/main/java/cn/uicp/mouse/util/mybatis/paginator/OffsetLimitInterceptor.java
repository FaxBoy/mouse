package cn.uicp.mouse.util.mybatis.paginator;
...
import cn.uicp.mouse.util.mybatis.paginator.dialect.Dialect;
import cn.uicp.mouse.util.mybatis.paginator.domain.PageBounds;
import cn.uicp.mouse.util.mybatis.paginator.domain.PageList;
import cn.uicp.mouse.util.mybatis.paginator.domain.Paginator;
import cn.uicp.mouse.util.mybatis.paginator.support.PropertiesHelper;
import cn.uicp.mouse.util.mybatis.paginator.support.SQLHelp;
        ...
        if(pageBounds.isContainsTotalCount()){
            Callable<Paginator> countTask = new Callable() {
                public Object call() throws Exception {
                	BoundSql bsq =ms.getBoundSql(parameter) ;
                    Integer count;
                    Cache cache = ms.getCache();
                    if(cache != null && ms.isUseCache() && ms.getConfiguration().isCacheEnabled()){
                        CacheKey cacheKey = executor.createCacheKey(ms,parameter,new PageBounds(),copyFromBoundSql(ms,bsq,dialect.getCountSQL(), bsq.getParameterMappings(), bsq.getParameterObject()));
                        count = (Integer)cache.getObject(cacheKey);
                        if(count == null){
                            count = SQLHelp.getCount(ms,parameter,bsq,dialect);
            ...
            countFutrue = call(countTask, async);
            if (countFutrue.get().getTotalCount() > 50000) {
            	isInterception =true;
            }
        }
    ...
    private MappedStatement copyFromNewSql(MappedStatement ms, BoundSql boundSql,
    	...
    	if (isInterception) {
    		if (sql.indexOf("order") != -1) {
    			sql=sql.substring(0,sql.indexOf("order"))+sql.substring(sql.indexOf("order"));
			}else{
				sql=sql.substring(0,sql.indexOf("where")+5)+sql.substring(sql.indexOf("where")+5);
			}
